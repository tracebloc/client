...
| Host | Why |
|---|---|
| `registry-1.docker.io` (Docker Hub) | k3s and busybox; the tracebloc-published images — the control plane, the training images and mysql-client — only if you roll them back with `images.traceblocRegistry=docker.io` |
| `ghcr.io` (+ `pkg-containers.githubusercontent.com`, where GHCR redirects layer downloads) | the tracebloc control-plane images (jobs-manager, pods-monitor, resource-monitor, requests-proxy), the training images jobs-manager spawns and `mysql-client` — all the default since the GHCR migration — + the ingestor image + k3d node images |
...
> **`TRACEBLOC_CA_BUNDLE` is not installer-only.** The standalone scripts under `scripts/` that reach the public internet — `chart-repo-health.sh`, `check-facts.sh`, `check-install-surface.sh`, `list-images.sh`, `resolve-training-digests.sh` and `verify-published-index.sh` — read the same variable and map it onto `CURL_CA_BUNDLE` for their own fetches, with no install involved. So the paragraph above covers you whether you are installing or just running one of those checks; if one of them fails with `x509: certificate signed by unknown authority`, this is the knob. **Only if unset:** an explicit `CURL_CA_BUNDLE` always wins, because these variables *replace* the trust store rather than adding to it — so if you set both, the `CURL_CA_BUNDLE` one is used and the `TRACEBLOC_CA_BUNDLE` one is inert. Those scripts say so by name when a fetch fails that way.
...
**Moving only the tracebloc images.** `global.imageRegistry` re-homes *everything*. The tracebloc-published images — the control-plane images (jobs-manager, pods-monitor, resource-monitor, requests-proxy) and the training images jobs-manager spawns (through `JOB_IMAGE_HOST`, the registry prefix it stamps onto every training image) — pull from `ghcr.io` by default. `docker.io` still serves the tags it last received — Docker Hub is a frozen archive, published to no longer, so it holds older images and none of the digests resolved on `ghcr.io` since. To point just those images at a different registry, set `images.traceblocRegistry` (a bare host) and leave `global.imageRegistry` unset; `global.imageRegistry` always wins when both are set. Rolling them back to Docker Hub is one flag — `--set images.traceblocRegistry=docker.io` — which moves the control plane and the training-image host together, and, being user-supplied, persists across the fleet auto-upgrade until you clear it. The image-refresh CronJob follows the same value and can resolve digests anonymously on `ghcr.io` and `docker.io` only; any other registry makes the reconcile inert, exactly as a mirror does. `tracebloc/mysql-client` moves with this knob too — it is still frozen and digest-pinned, but it is published to `ghcr.io` like the rest, so one flag rolls the whole tracebloc set back together. The ingestor does not move with it (named by full repository, already on `ghcr.io`). A control-plane digest pin you set yourself (`images.<image>.digest`) is honoured only on the registry it was resolved on — declare it in `images.<image>.digestRegistry`, or the pin is ignored and the workload floats on the tag with a NOTES warning (see `client/MIGRATION.md`, 1.9.119). The training-image digest map follows the same rule: it is rendered only when `images.training.digestRegistry` names the registry the training pods pull from, so on a mirror or after the `docker.io` rollback the training pods float on their tag.

**Re-homing one third-party image under a different path.** The helper images (`alpine/*`, `ubuntu/squid`, `busybox`, the ROCm device plugin) are pinned by digest, and a digest is applied only to the `repository:tag` it was resolved for. Overriding `<site>.repository` or `<site>.tag` therefore renders your `repository:tag` and NOTES warns that the chart pin was not applied; set your own `<site>.digest` to pin it. If your mirror serves the *same* bytes under another path (a proxy cache), keep the chart pin by declaring its new identity: `<site>.digestFor: "<your repository>:<tag>"` (see `client/MIGRATION.md`, 1.9.146).
...
The `tracebloc-resource-monitor` DaemonSet mounts `hostPath` volumes (`/proc`, `/sys`, and the filesystems the client stores on — `resourceMonitor.hostStoragePath`, default `/`, plus a second mount of `hostPath.datasetPath` when datasets were relocated off the local tree; all read-only and used only for a `statvfs` free/total reading; see [SECURITY.md §6.6](SECURITY.md)) which Pod Security Admission's `restricted` profile bans outright. The chart isolates it in a dedicated **privileged** namespace (default `tracebloc-node-agents`) so it does not constrain the restricted profile on the release namespace.
