...
> - **Linux (native Docker):** `sudo cp <corporate-ca>.pem /usr/local/share/ca-certificates/tracebloc-corp-ca.crt && sudo update-ca-certificates && sudo systemctl restart docker`.
> - **Docker Desktop (macOS / Windows / Docker Desktop for Linux):** the daemon runs in a VM the installer can't reach. Trust the CA in the host OS store — the **macOS keychain** (set "Always Trust") or the **Windows Trusted Root** store (`certlm.msc`) — then restart Docker Desktop, which reads the host store on start. See [docs.docker.com](https://docs.docker.com/).
