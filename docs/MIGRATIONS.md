...
1. **`--force-conflicts` re-takes *every* field the chart renders that a non-Helm manager owns — not only `limits.cpu`.** Helm prints the full conflict list on the failed attempt; read it first. On a long-running fleet the list can include `image-refresh`'s pinned digests (`kubectl set image`); forcing reverts them to the chart's rendered image, and `image-refresh` re-pins on its next tick. That is recoverable but is a real, if brief, image churn — don't force blind.
   ...
   **From chart 1.9.136 (#1013 / client-runtime#199) the `.image` conflict narrows.** The chart now renders the digest the workload is **already running**: the control-plane image sites go through `tracebloc.controlPlaneImage`, whose seed (`tracebloc.controlPlaneSeedDigest`) reads the **LIVE workload spec** via `lookup` — not image-refresh's annotation. So in the **steady state** Helm applies the same value the `kubectl-set` manager holds and the `.spec…containers[…].image` conflict disappears. **This whole note is about the hand-run server-side path** (`helm upgrade --server-side=true --force-conflicts`) that reports a `.image` conflict at all — the automated fleet path is Helm 3.16 client-side (`alpine/helm:3.16.4`, no `--server-side`), whose 3-way merge **overwrites** the live value rather than conflicting, so it never surfaces here.

   On the server-side path, a `.image` conflict on 1.9.136+ means the chart rendered `:tag` while the live spec carries a digest — i.e. the seed did **not** fire. It is not annotation-vs-live lag, because the annotation is not what the chart reads. Diagnose before you force, because the seed is gated on three things: the live ref's registry must match `tracebloc.tbRegistry` (a mutating webhook that rewrites the registry prefix makes the live ref unrecognisable to the seed), image-refresh must be enabled, and its registry must be resolvable. Check all three. **Forcing here is the one case where `--force-conflicts` is *not* recoverable-by-next-tick**: it reverts the workload to `:tag`, and on a non-resolvable registry there is no next tick to re-pin it.
