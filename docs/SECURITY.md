...
**Datasets on network storage (`HOST_DATASET_DIR`) are a known gap.** That installer flag bind-mounts the customer's dataset volume at `/tracebloc-data` and points the dataset PV there, deliberately keeping datasets off the local disk (§5.4). Nothing sets `resourceMonitor.hostStoragePath` in that flow, so the meter keeps measuring the node root — the disk the datasets are specifically *not* on — and "am I about to run out of room for the next ingest?" is answered about the wrong volume. Set `resourceMonitor.hostStoragePath` to the dataset mount on those installs. Wiring it automatically is tracked separately; it is a product question (the meter is specified as the client's whole storage, not the dataset volume) rather than an oversight.
