...
| Network | Default-deny ingress; egress only to cluster DNS, the in-cluster MySQL, the requests-proxy and the egress gateway, so external HTTPS goes only through the gateway's FQDN allowlist | Kubernetes `NetworkPolicy` + squid egress gateway |
...
Node-level agents (`tracebloc-resource-monitor` DaemonSet) run in a separate namespace (`tracebloc-node-agents`) at `enforce: privileged` — they legitimately need hostPath access to `/proc` / `/sys` / cgroups, **and to the filesystems the client stores on** (`resourceMonitor.hostStoragePath`, default `/`, plus the relocated dataset tree where there is one — see below). The release namespace stays clean.

Those last mounts are a different kind from the other two and are called out separately for that reason: `/proc` and `/sys` are kernel metric interfaces, while these are the node's real filesystems. They back the cockpit's Storage meter, which reports the whole storage available to the client rather than the dataset PVC (backend#3762). The posture is **read-only**, non-root (the pod sets `runAsNonRoot: true`, with `allowPrivilegeEscalation: false` and `capabilities.drop: [ALL]` on the container; on OpenShift the SCC additionally enforces `MustRunAsNonRoot`, but `openshift.scc.enabled` defaults to `false`, so on AKS/EKS/k3s/bare-metal the pod's own context is what holds), and the agent only ever calls `statvfs` on each mount root — it reads free/total block counts and never opens, lists, or transmits file contents. `statvfs` has no narrower primitive; measuring a filesystem requires a path on it.

Operators who would rather not expose `/` can point `resourceMonitor.hostStoragePath` at a narrower mount, at the cost of the meter describing that volume instead of the node. Setting it to a path that is not a mountpoint reports the filesystem containing it, not the subtree.

**If you narrow it, the path must exist on every node.** The mount is `type: Directory`, which requires the path to be present already, and the DaemonSet runs with `tolerations: [{operator: Exists}]` — so it schedules onto *every* node, control-plane included. Point it at a mount only the workers have and those pods sit in `ContainerCreating` indefinitely. There is no liveness or readiness probe here, so that shows up as a node quietly ceasing to report rather than as a crash loop with a restart count.

**Datasets on network storage (`HOST_DATASET_DIR`) are measured too (backend#3916).** That installer flag bind-mounts the customer's dataset volume at `/tracebloc-data` and points the dataset PV there, deliberately keeping datasets off the local disk (§5.4) — and datasets are the bytes that grow without anyone deciding to grow them. A single measured path cannot describe such a client: `/` misses the datasets, and the dataset mount misses mysql, logs and experiment scratch. So the chart measures **both**. Whenever `hostPath.datasetPath` names something other than the local default, the DaemonSet gains a second read-only mount of that path at `/host/dataset` and passes it to the collector as `HOST_DATASET_PATH`; the collector sums the two after de-duplicating by backing device, so a dataset tree that shares a disk with the root mount is counted once rather than doubling the reported capacity. It needs no operator action: both installers already set `hostPath.datasetPath` when `HOST_DATASET_DIR` is given.

Two consequences worth knowing. The second mount widens what the agent can `statvfs` to the dataset volume — the same read-only, free/total-block-counts-only posture as the root mount, and no file contents either way. And **the same every-node rule applies**: it is `type: Directory`, deliberately not `DirectoryOrCreate`, because conjuring a missing dataset directory would hand the meter an empty folder on the root disk and report it, confidently, as the dataset volume. Both installers bind that volume on every node, so the flow that sets the key satisfies this; a hand-written `hostPath.datasetPath` pointing at a path only some nodes have parks resource-monitor in `ContainerCreating` there, exactly as narrowing `hostStoragePath` does. Finally, a path the agent cannot read drops that node from **both** figures rather than contributing a zero — a partial sum would be the original defect one level down, a healthy-looking percentage of the wrong disk. The per-category breakdown (datasets vs. experiments vs. database as separate figures) is tracked separately as backend#3917; this is capacity and consumption for the client as a whole.
...
### 8.2 Training-pod outbound HTTPS (G2) — **deny-by-default shipped (1.9.96); tracebloc template fleets sealed + probe-verified (2026-09-08)**
...
> **Status (2026-09-08): the three tracebloc template fleets are sealed and enforcement-probe-verified** — dev (`tracebloc-templates` on `tb-client-dev-templates`), staging (`tracebloc-templates-stg`) and prod (`tracebloc-templates-prod`), the latter two on `tracebloc-clients-prod`. On each, the rendered training NetworkPolicy allows egress only to DNS + mysql(3306) + requests-proxy(8888) + egress-proxy(3128) — **no direct `0.0.0.0/0:443`** — and a training-labelled probe pod was **BLOCKED** on direct `1.1.1.1:443` and `huggingface.co:443` (after the VPC-CNI standard-mode reconcile window, ~≤16s), with HuggingFace also `403`-denied through the squid allowlist. Each fleet then ran a real `image_classification` experiment to **COMPLETED** through the gateway, its spawned training pod carrying `HF_HUB_OFFLINE`/`TRANSFORMERS_OFFLINE`/`HF_DATASETS_OFFLINE=1` and `HTTPS_PROXY=egress-proxy-service:3128`. The HF runtime-fetch gate (client-runtime#416 / backend#1501) that previously **held** these mixed NLP-inclusive fleets is resolved: the jobs-manager now injects the offline flags, so an NLP template that would have runtime-fetched HuggingFace fails **closed at the library layer** — the clean "closed door" — rather than by an opaque network block. Per-fleet evidence (netpol dumps, probe time-series, run ids) is on client-runtime#199; the fleet records are in [SEAL-CHECK.md](SEAL-CHECK.md). **New installs and any customer fleet still follow the per-fleet rollout below** — Gate 0 (CNI enforcement pre-flight) is non-negotiable, since the default only *enforces* where the CNI enforces egress NetworkPolicy.

**Mechanism (chart 1.7.0, client-runtime#102):** an in-cluster **egress gateway** (`egressProxy`, a squid forward proxy) permits HTTPS CONNECT only to the FQDNs on `egressProxy.allowlist`, which are the tracebloc backend API host of each environment and the X-Ray backend, nothing else. App Insights is not on the gateway allowlist any more: the edge stopped exporting there (client-runtime#471), the backend stopped handing out the credential (backend#3078), and `client/tests/egress_proxy_test.yaml` asserts its absence. Behind a corporate proxy the gateway chains upstream via `cache_peer`. With routing on, jobs-manager injects `HTTPS_PROXY=egress-proxy-service:3128` into each training pod (and drops the raw `HTTP_PROXY_HOST`), so backend traffic flows through the gateway; Service Bus already goes via the requests-proxy. The pod then needs no direct internet, and the external-443 rule can be dropped. `scripts/tests/egress-prose-agreement.sh` derives the allowlist and the two egress defaults from `client/values.yaml` and reddens on any document here that contradicts them.
...
- **2026-09-08** — Recorded the egress seal as **sealed + enforcement-probe-verified** across all three tracebloc template fleets — dev (`tb-client-dev-templates`), staging + prod (`tracebloc-clients-prod`) — in §8.2, and corrected the §5.1 staging-fleet cluster attribution to match (client-runtime#199). The hold lifted because the HF runtime-fetch gate now ships `HF_HUB_OFFLINE`/`TRANSFORMERS_OFFLINE`/`HF_DATASETS_OFFLINE`, so an NLP template fails closed at the library layer. New/customer fleets still follow the §8.2 gated rollout. Fleet records in [SEAL-CHECK.md](SEAL-CHECK.md).
- **2026-09-18** — Corrected §8.2, which still said the egress gateway allowlist permits App Insights although it no longer does (client-runtime#471 / backend#3078; `client/tests/egress_proxy_test.yaml` asserts the absence). Corrected the TL;DR network row, which still read as if direct external HTTPS were the default. Added `scripts/tests/egress-prose-agreement.sh`, which derives the egress defaults and the allowlist from `client/values.yaml` and reddens on any document that contradicts them, with `egress-prose-mutations.sh` proving it can fail.
