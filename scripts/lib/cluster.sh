...
# or host runs outside every node container. The generator's header states the
# headroom policy; the record states the numbers it was applied to.
#
...
# The platform key the reservation table is indexed by: `darwin` or `linux`, the
# lower-cased kernel name. A function, so the bats suite can drive the writer as
# either platform (and as one with no record) on whatever host runs the tests.
_kubelet_reservation_platform() { uname -s | tr '[:upper:]' '[:lower:]'; }
...
_create_new_cluster() {
  ...
  if ! _kubelet_reservation_measured "$(_kubelet_reservation_platform)"; then
    warn "No measured node reservation exists for $(uname -s) yet, so this node's allocatable will equal its capacity -- the training envelope is sized against a number that includes the kubelet and container runtime."
