...
# guard itself and any line that opted out. Prints file:line:text. grep exit 2+
# (bad regex/flags/tree) is an INTERNAL error → fail closed, never silent-pass.
scan() {
  ...
  printf '%s' "$out" | grep -vE '# *style-guard: *allow' || true
}
