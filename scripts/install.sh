...
# (tracebloc/client), but the release is built and cosign-signed in the private
# source repo (tracebloc/client-dev, renamed from tracebloc/client on 2026-09-17),
# and the keyless certificate's SAN names the repo that RAN the release workflow:
# every release cut before the rename carries the old name, every release after
# it the new one. Trust exactly those two — anything else is rejected — so both
# the last pre-rename stable and the first post-rename one verify. A verifier
# pinned to the old name alone fails every post-rename release with "signature
# verification FAILED"; the CLI shipped that live (backend#3985, backend#3601 F).
# ONE declaration: install.ps1 carries the same set as $SignerRepos and a test on
# each side holds the two in lockstep.
SIGNER_REPOS='client|client-dev'
# The identity, derived from that set and narrowed to the exact release workflow
# on a tag ref: every signed release to date (sampled v1.8.2 … v1.9.123 against the
# published certs on 2026-09-17) carries
#   https://github.com/tracebloc/<repo>/.github/workflows/release-helm-chart.yaml@refs/tags/vX.Y.Z
# Anchored at the start so a look-alike host or org cannot prefix its way in;
...
verify_manifest_signature() {
  ...
  # The keyless signing identity: the release workflow's OIDC cert, from the ONE
  # declaration above (SIGNER_ID_RE — both repo names, exact workflow, tag ref).
  # Shared by both the offline-bundle and the online sig/cert paths below.
  local id_re="$SIGNER_ID_RE"
