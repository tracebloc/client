...
# ── kubelet config drop-in: the node reservation (backend#2460) ──────────────
#
# The writer is driven as a MEASURED platform and as one with no record, on
# whatever host runs the suite, by overriding _kubelet_reservation_platform. The
# expected values are read from the twin's OWN generated block (by the same
# indirection the writer uses), never typed here: the numbers are the generator's
# to decide, this suite pins that what is declared is what is written.

_first_measured_platform() { set -- $TB_KUBELET_RESERVATION_PLATFORMS; printf '%s' "${1:-}"; }

@test "kubelet reservation: a measured platform writes kubeReserved, systemReserved and the eviction threshold from the embed" {
  local plat; plat="$(_first_measured_platform)"
  [ -n "$plat" ] || skip "no platform has a measured record in this tree"
  _kubelet_reservation_platform() { printf '%s' "$plat"; }
  local key cpu mem sys
  key="$(printf '%s' "$plat" | tr '[:lower:]' '[:upper:]')"
  cpu="TB_KUBELET_KUBE_RESERVED_CPU_MILLI_${key}"; mem="TB_KUBELET_KUBE_RESERVED_MEM_MIB_${key}"; sys="TB_KUBELET_SYSTEM_RESERVED_MEM_MIB_${key}"
  run _write_kubelet_config
  [ "$status" -eq 0 ] || { echo "$output"; return 1; }
  run cat "$output"
  [[ "$output" == *"enforceNodeAllocatable:"*"- pods"* ]] || { echo "$output"; return 1; }
  [[ "$output" == *"kubeReserved:"*"  cpu: ${!cpu}m"*"  memory: ${!mem}Mi"* ]] || { echo "$output"; return 1; }
  [[ "$output" == *"systemReserved:"*"  memory: ${!sys}Mi"* ]] || { echo "$output"; return 1; }
  [[ "$output" == *"evictionHard:"*"  memory.available: ${TB_KUBELET_EVICTION_MEM_MIB}Mi"* ]] || { echo "$output"; return 1; }
  # ONLY memory.available: k3s's disk keys are merged in from its own drop-in
  # (measured), and restating them here would pin k3s's numbers in a second place.
  [[ "$output" != *"imagefs.available"* ]] || { echo "restates k3s's disk threshold: $output"; return 1; }
  [[ "$output" != *"nodefs.available"* ]] || { echo "restates k3s's disk threshold: $output"; return 1; }
  # The image-GC half is still there -- one file, both tickets.
  [[ "$output" == *"imageGCHighThresholdPercent: ${TB_KUBELET_IMAGE_GC_HIGH_PERCENT}"* ]] || { echo "$output"; return 1; }
}

@test "kubelet reservation: the YAML a measured platform writes carries no tabs and every value is a k8s quantity" {
  local plat; plat="$(_first_measured_platform)"
  [ -n "$plat" ] || skip "no platform has a measured record in this tree"
  _kubelet_reservation_platform() { printf '%s' "$plat"; }
  run _write_kubelet_config
  [ "$status" -eq 0 ] || return 1
  run grep -c $'\t' "$output"
  [ "$output" = "0" ] || { echo "tabs in YAML: $output"; return 1; }
  run _write_kubelet_config
  # Every reservation line is `key: <digits>(m|Mi)` -- an empty embed would have
  # produced `memory: Mi`, which the kubelet refuses to start on.
  run grep -E '^\s+(cpu|memory|memory\.available): ' "$output"
  [ "$status" -eq 0 ] || return 1
  while IFS= read -r line; do
    [[ "$line" =~ :\ [1-9][0-9]*(m|Mi)$ ]] || { echo "not a quantity: '$line'"; return 1; }
  done <<<"$output"
}

@test "kubelet reservation: a platform with NO record writes no reservation at all (never a borrowed one)" {
  _kubelet_reservation_platform() { printf 'nowhere'; }
  run _write_kubelet_config
  [ "$status" -eq 0 ] || { echo "$output"; return 1; }
  run cat "$output"
  [[ "$output" != *"kubeReserved"* ]] || { echo "$output"; return 1; }
  [[ "$output" != *"systemReserved"* ]] || { echo "$output"; return 1; }
  [[ "$output" != *"evictionHard"* ]] || { echo "$output"; return 1; }
  [[ "$output" != *"enforceNodeAllocatable"* ]] || { echo "$output"; return 1; }
  # And the image-GC file it always wrote is intact.
  [[ "$output" == *"imageGCHighThresholdPercent: ${TB_KUBELET_IMAGE_GC_HIGH_PERCENT}"* ]] || { echo "$output"; return 1; }
}

@test "kubelet reservation: a measured platform whose embed is broken REFUSES to write (fail closed)" {
  # Listed as measured, but the value the writer reads is empty: writing on would
  # produce `memory: Mi` (kubelet refuses to start -> a node that is never Ready)
  # or `0` (a node that looks reserved and is not). Neither is better than stopping.
  ...
  _kubelet_reservation_platform() { printf 'broken'; }
  run _write_kubelet_config
  [ "$status" -ne 0 ] || { echo "wrote a file from a blank value: $output"; return 1; }
  [[ "$output" == *"TB_KUBELET_KUBE_RESERVED_MEM_MIB_BROKEN"* ]] || { echo "does not name the broken variable: $output"; return 1; }
  # A zero is refused too: it is not a reservation, it is the status quo in disguise.
  TB_KUBELET_KUBE_RESERVED_MEM_MIB_BROKEN=0
  run _write_kubelet_config
  [ "$status" -ne 0 ] || { echo "accepted a zero reservation: $output"; return 1; }
}

@test "kubelet reservation: create says so, once, when this platform has no measured record" {
  _kubelet_reservation_platform() { printf 'nowhere'; }
  run _create_new_cluster
  [ "$status" -eq 0 ] || { echo "$output"; return 1; }
  [[ "$output" == *"No measured node reservation exists for"* ]] || { echo "$output"; return 1; }
  [[ "$output" == *"measure-node-reservation.sh"* ]] || { echo "no pointer to the harness: $output"; return 1; }
}

@test "kubelet reservation: create is silent about it when this platform IS measured" {
  local plat; plat="$(_first_measured_platform)"
  [ -n "$plat" ] || skip "no platform has a measured record in this tree"
  _kubelet_reservation_platform() { printf '%s' "$plat"; }
  run _create_new_cluster
  [ "$status" -eq 0 ] || { echo "$output"; return 1; }
  [[ "$output" != *"No measured node reservation exists"* ]] || { echo "warned on a measured platform: $output"; return 1; }
}

@test "existing cluster: mount present but the file predates the reservation -> warns with a recreate hint (measured platform)" {
  local plat; plat="$(_first_measured_platform)"
  [ -n "$plat" ] || skip "no platform has a measured record in this tree"
  _kubelet_reservation_platform() { printf '%s' "$plat"; }
  docker() { if [[ "$1" == inspect ]]; then printf '/tracebloc\n%s\n' "$TB_KUBELET_CONFIG_NODE_PATH"; return 0; fi; command docker "$@"; }
  mkdir -p "$(dirname "$(_kubelet_config_path)")" || return 1
  printf 'apiVersion: kubelet.config.k8s.io/v1beta1\nkind: KubeletConfiguration\nimageGCHighThresholdPercent: 75\n' > "$(_kubelet_config_path)" || return 1
  run _check_existing_cluster_kubelet_config
  [ "$status" -eq 0 ] || { echo "refused instead of warning: $output"; return 1; }
  [[ "$output" == *"carries no node reservation"* ]] || { echo "$output"; return 1; }
  [[ "$output" == *"allocatable still equals capacity"* ]] || { echo "$output"; return 1; }
  [[ "$output" != *"stock 85%"* ]] || { echo "also fired the image-GC warning on a mounted file: $output"; return 1; }
}

@test "existing cluster: mount present and the file carries the reservation -> silent" {
  local plat; plat="$(_first_measured_platform)"
  [ -n "$plat" ] || skip "no platform has a measured record in this tree"
  _kubelet_reservation_platform() { printf '%s' "$plat"; }
  docker() { if [[ "$1" == inspect ]]; then printf '/tracebloc\n%s\n' "$TB_KUBELET_CONFIG_NODE_PATH"; return 0; fi; command docker "$@"; }
  _write_kubelet_config >/dev/null || return 1
  run _check_existing_cluster_kubelet_config
  [ "$status" -eq 0 ] || return 1
  [[ "$output" != *"no node reservation"* ]] || { echo "warned on a reserved cluster: $output"; return 1; }
}

@test "existing cluster: pre-reservation file on an UNMEASURED platform -> silent (nothing to recreate into)" {
  _kubelet_reservation_platform() { printf 'nowhere'; }
  docker() { if [[ "$1" == inspect ]]; then printf '/tracebloc\n%s\n' "$TB_KUBELET_CONFIG_NODE_PATH"; return 0; fi; command docker "$@"; }
  mkdir -p "$(dirname "$(_kubelet_config_path)")" || return 1
  ...
  run _check_existing_cluster_kubelet_config
  [ "$status" -eq 0 ] || return 1
  [[ "$output" != *"no node reservation"* ]] || { echo "$output"; return 1; }
}

@test "existing cluster: mount present but the host file is unreadable -> silent (cannot tell is not missing)" {
  local plat; plat="$(_first_measured_platform)"
  [ -n "$plat" ] || skip "no platform has a measured record in this tree"
  _kubelet_reservation_platform() { printf '%s' "$plat"; }
  docker() { if [[ "$1" == inspect ]]; then printf '/tracebloc\n%s\n' "$TB_KUBELET_CONFIG_NODE_PATH"; return 0; fi; command docker "$@"; }
  rm -f "$(_kubelet_config_path)"
  run _check_existing_cluster_kubelet_config
  [ "$status" -eq 0 ] || return 1
  [[ "$output" != *"no node reservation"* ]] || { echo "warned on a file it could not read: $output"; return 1; }
}
