...
$Mutations = @(
  @{ Name  = 'the reboot prompt asks even with nobody at the console (backend#2675)'
     ...
     After = 'function Read-RebootChoice {'
...
function Get-MarkerIndex {
    param([string[]]$Lines, [string]$Find, [string]$After)
    $from = 0
    if ($After) {
    ...
    $hits = @($from..($Lines.Count - 1) | Where-Object { $Lines[$_] -eq $Find })
    if ($hits.Count -eq 0) { return @{ Error = "no line matches -- the code moved and this mutation now proves nothing" } }
    if (-not $After -and $hits.Count -gt 1) { return @{ Error = "$($hits.Count) lines match -- add an `After` anchor to disambiguate" } }
    return @{ Index = $hits[0] }
}

function Resolve-Marker {
    param([string]$Path, [string]$Find, [string]$After)
    $lines = [System.IO.File]::ReadAllLines($Path)
    $r = Get-MarkerIndex -Lines $lines -Find $Find -After $After
...
"@
    $out = & pwsh -NoProfile -Command $script 2>$null
    $line = @($out | Where-Object { $_ -like 'RESULT *' }) | Select-Object -Last 1
    if (-not $line) { return @{ PassedCount = 0; FailedCount = -1; Failed = @('the suite produced no result line') } }
...
foreach ($m in $Mutations) {
    ...
    try {
        ...
        Copy-Item (Join-Path $RepoRoot 'scripts') (Join-Path $sandbox 'scripts') -Recurse -Force
        $target = Join-Path $sandbox $m.File
        $lines  = [System.IO.File]::ReadAllLines($target)
        $idx    = (Get-MarkerIndex -Lines $lines -Find $m.Find -After ($(if ($m.ContainsKey('After')) { $m.After } else { '' }))).Index
        $lines[$idx] = $m.Repl
        [System.IO.File]::WriteAllLines($target, $lines)
        $mr = Invoke-Suite -Root $sandbox -Suite $m.Suite
        $fails = $mr.FailedCount
        if ($fails -gt 0) { $caught++; Write-Host ("  caught   {0}  ({1} failing)" -f $m.Name, $fails) -ForegroundColor Green }
        else { $survived += $m.Name; Write-Host ("  SURVIVED {0}" -f $m.Name) -ForegroundColor Red }
    } finally {
...
Write-Host ""
Write-Host ("{0}/{1} mutations caught" -f $caught, $Mutations.Count)
