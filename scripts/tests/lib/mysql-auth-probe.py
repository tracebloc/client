...
#   * a multi-MB LONGBLOB round-trips through the driver (max_allowed_packet is
#     reported, not asserted — see backend#3032).
#
...
def main() -> int:
    ...
    # max_allowed_packet is REPORTED, not asserted: the chart's 256M ConfigMap
    # applies locally but not under CI k3d (backend#3032), and it is a shared
    # chart concern, not this test's native-arm64 + auth close condition. The
    # LONGBLOB below (16MB, under even the 64M default) still proves the driver
    # round-trips a real multi-MB payload.
