...
# The node reservation (backend#2460): kubeReserved / systemReserved / the memory
# eviction threshold, so `allocatable` stops meaning "the whole machine". The
# bash twin's block in scripts/lib/cluster.sh carries the full rationale. The
# values are DERIVED from per-platform measurement records
# (scripts/spec/node-reservation/, written by scripts/tests/measure-node-reservation.sh)
# by scripts/gen-node-reservation-embed.sh, whose --check runs in the required
# Source-of-truth drift job; kubelet-config-agreement.sh holds the two twins'
# blocks equal. A platform with NO record gets NO reservation -- `windows` appears
# in TB_KUBELET_RESERVATION_PLATFORMS only once a WSL2 record exists, and until
# then Write-KubeletConfig writes the image-GC file it always wrote and the
...
# The platform key this installer writes for. It only ever runs on Windows (the
# k3d nodes live in the WSL2 VM); a parameter, so the Pester suite can drive
# Write-KubeletConfig as a measured platform and as an unmeasured one.
function Get-KubeletReservationPlatform { return "windows" }

function Test-KubeletReservationMeasured($Platform) {
  return ((" $TB_KUBELET_RESERVATION_PLATFORMS ") -like "* $Platform *")
}

# The reservation values for a measured platform, read from the generated block
# by name; $null -- with the reason logged -- when any is missing or not a
# positive whole number. A platform listed as measured whose numbers cannot be
# read is a broken embed, and `memory: Mi` is a kubelet that refuses to start.
# Mirrors bash _kubelet_reservation_values.
function Get-KubeletReservationValues($Platform) {
  $key = $Platform.ToUpper()
  $names = @("TB_KUBELET_KUBE_RESERVED_CPU_MILLI_$key", "TB_KUBELET_KUBE_RESERVED_MEM_MIB_$key",
  ...
  return @{ KubeCpuMilli = $vals["TB_KUBELET_KUBE_RESERVED_CPU_MILLI_$key"]
            KubeMemMib   = $vals["TB_KUBELET_KUBE_RESERVED_MEM_MIB_$key"]
            SystemMemMib = $vals["TB_KUBELET_SYSTEM_RESERVED_MEM_MIB_$key"]
            EvictionMib  = $vals["TB_KUBELET_EVICTION_MEM_MIB"] }
}

# Write the kubelet config drop-in the k3d nodes are pointed at (backend#2634 +
# backend#2460). Returns the path. Throws when it cannot write, or when the
# platform is listed as measured but its numbers will not read (fail closed).
...
# YAML parser rejects the BOM -- so the node would fail to start with a message
# about the file, not about us. Mirrors bash _write_kubelet_config.
function Write-KubeletConfig($Path, $Platform) {
  ...
  if (Test-KubeletReservationMeasured $Platform) {
    ...
    if (-not $r) { throw "the node reservation for '$Platform' is listed as measured but its values cannot be read (see the log)" }
    # `memory.available` is the ONLY evictionHard key written: k3s's disk keys
    # are merged in from its own kubelet.conf.d drop-in (measured on the bash
    # side, 2026-09-09), and restating them would pin k3s's numbers here too.
    $lines += @(
      "enforceNodeAllocatable:",
      "- pods",
      "kubeReserved:",
      "  cpu: $($r.KubeCpuMilli)m",
      "  memory: $($r.KubeMemMib)Mi",
      "systemReserved:",
      "  memory: $($r.SystemMemMib)Mi",
      "evictionHard:",
      "  memory.available: $($r.EvictionMib)Mi"
    )
  }
...
# remove_by 2026-12-31; both are read).
function Set-ClusterAutostart {
  if ($env:TRACEBLOC_SKIP_AUTOSTART -or $env:TRACEBLOC_NO_AUTOSTART) { return }
...
function Test-ExistingClusterKubeletConfig {
  ...
  # reservation. The kubelet reads its config at start, so rewriting the host file
  # would arm a smaller allocatable on the next node restart under pods sized
  # against the old one -- advise, do not rewrite. Only on a platform that HAS a
  # measured reservation, and silent when the mounts or the file could not be read
  # ("cannot tell" must not read as "missing"). Mirrors the bash twin.
  if (-not $kubeletMounts) { return }
  if (-not (Test-KubeletReservationMeasured (Get-KubeletReservationPlatform))) { return }
  $cfg = Join-Path (Join-Path $HOST_DATA_DIR "kubelet") "kubelet.yaml"
  $content = $null
  try { $content = Get-Content -LiteralPath $cfg -Raw -ErrorAction Stop } catch { return }
  if ($content -and ($content -notmatch '(?m)^kubeReserved:')) {
...
function New-K3dCluster {
  ...
  if ($clusterExists) {
    ...
    # kubelet replaces WHOLESALE when they arrive as `--kubelet-arg` CLI flags, so
    # that path silently drops k3s's disk thresholds. Through this file they are
    # MERGED key by key instead: k3s copies it into its own kubelet.conf.d as
    # 10-cli-config.conf, after 00-k3s-defaults.conf (measured 2026-09-09, see the
    # bash twin). Image GC is scalar and would survive the CLI, but belongs beside
    # the reservation it interacts with. Stock 85/80 leaves a 5-point band, which
    # on a real disk can be smaller than ONE 2.7-11 GB task image.
    ...
    try {
      $null = Write-KubeletConfig -Path $kubeletCfgPath
    } catch {
    ...
    if (-not (Test-KubeletReservationMeasured (Get-KubeletReservationPlatform))) {
      Warn "No measured node reservation exists for Windows/WSL2 yet, so this node's allocatable will equal its capacity -- the training envelope is sized against a number that includes the kubelet and container runtime."
...
# `control-plane-footprint.sh --print-transient` (backend#2460). Mirrors bash
# _TB_CP_TRANSIENT_*; the rung derivation adds it to the VM minimum.
...
# Show-MemoryStatus reads it; no threshold is derived from it.
$script:TbLadderRungMemGib = 4
...
function Show-MemoryStatus {
  ...
  elseif ($effective -lt $warnMemGb) {
    # WHAT A TRAINING RUN NEEDS IS COMPOSED ONCE, AND EVERY ARM BELOW PRINTS IT.
    # The host-too-small arm used to carry its own figure ("too little to train
    # locally (~8 GB/job)"), and it went stale the moment the budget became
    # DERIVED from the generated VM minimum: its siblings named the rung and the
    # derived budget while a 5-6 GB host -- the only host that reaches that arm --
    # was still being sent after an 8 GB machine this same function says cannot
    # hold the rung. Bugbot caught it at both promotion hops of client#1093 and
    # client#1094. So no arm restates either number: they read this sentence, and
    # the guard in install-k8s.Tests.ps1 asserts it against the rung declared in
    # scripts/gen-vm-budget-embed.sh and the budget from Get-PfWarnMemGb.
    $rungNeed = "the smallest training run ($script:TbLadderRungMemGib GiB) needs a $warnMemGb GB budget once the kubelet reservation, k3s addons, control plane and CronJobs are counted"
    ...
    if ($hostTooSmall) {
      Warn "Memory: $label$budgetNote - enough to run the client, but too little to train locally: $rungNeed."
    ...
    } elseif ($recTrain -lt $warnMemGb) {
      # NO "N GB recommended to train" BELOW THE NEED (Bugbot on client#1085, round 2):
      # graded on host RAM this branch fires only when the host is below the warn
      # tier, so the clamped recommendation (host - reserve) is always below the
      # budget the sentence has just named. A concrete memory= here would be one
      # the same sentence says cannot hold a training run. Same rule as the bash
